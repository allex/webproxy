...
function action_rewrite(conf, req, res) {
    ...
    // Forward remote resource
    if (file.indexOf('//') === 0 || /^https?:\/\//.test(file)) {
        var x = utils.request(file)
        x.on('error', function(err) { responseNotFound(res, err.message) })
...
function forwardResponse(request, sResponse, rResponse) {
    ...
    rResponse.setHeader('X-Remote-Address', request.socket.remoteAddress)

    // simple forward for binary response.
    if (request.method == 'HEAD' || !ext || utils.isBinary(ext)) {
        sResponse.pipe(rResponse);
        ...
        sResponse.on('data', function(chunk) { stream.write(chunk) })
...
function proxy() {
            ...
            if (!err) {
                ...
                fs.readFile(file, function(err, data) {
                    if (data.length > 0) {
                        ...
                        } catch (e) { log.error('Parse host file error: ' + e.message) }
                    }
                })
            } else {
                log.debug('File \'' + file + '\' was not found.')
    ...
    var ip = config.hostname, port = config.port
